...
(function seedArtworks() {
  const ARTWORK_VERSION = 'v2';
  if (localStorage.getItem('artworksVersion') !== ARTWORK_VERSION) {
    const seedData = [
      { title: 'The Chamelion', category: 'traditional', desc: 'This is a School Artwork that I made in our ART APPRECIATION Subject on February 20, 2024', tools: 'Copic Markers', image: 'Artworks/The Chamelion.jpg', dateAdded: 'April 25, 2026' },
      { title: 'Circles', category: 'digital', desc: "This mini WIP Artwork is based on My OC and the OC's of my Friends", tools: 'Medibang Paint', image: 'Artworks/Circles.png', dateAdded: 'April 25, 2026' },
      { title: 'Wikang Pinoy', category: 'traditional', desc: 'This is the Artwork that I made during the Buwan ng Wika on STI College Naga participating on the Poster Making Contest on August 30, 2024', tools: 'Copic Markers and Pencils', image: 'Artworks/Wikang Pinoy.jpg', dateAdded: 'April 25, 2026' },
      { title: 'Artist Boy', category: 'character', desc: 'I made this art when I was practicing my Digital Tracing Art back on 2021', tools: 'IbisPaintX', image: 'Artworks/Artist Boy.jpg', dateAdded: 'April 25, 2026' },
      { title: 'Lion and Dino', category: 'digital', desc: 'This Artwork is based on the OC of my friends as they wanted me to Commission it for them.', tools: 'Medibang Paint', image: 'Artworks/Lion and Dino.png', dateAdded: 'April 25, 2026' },
      { title: 'Solemn of Tranquility', category: 'traditional', desc: 'This artwork resembles the peaceful greenery fields and the serenity silents of the river, I made this canvas during the Art Exhibit on June 11, 2024 at STI College Pasay Edsa', tools: 'Canvas and Alcohol Markers', image: 'Artworks/Solemn of Tranquility.jpg', dateAdded: 'April 25, 2026' },
      { title: 'Family Collage', category: 'collage', desc: 'I created this Collage during the Holliweek event of Prosesyon in Calabanga Camarines sur on April 3, 2026', tools: 'Sony Camera and SCRL App', image: 'Artworks/Family Collage.jpg', dateAdded: 'April 25, 2026' },
      { title: 'Athena', category: 'traditional', desc: 'This Artwork is based on Greek Goddess Athena that I made during INKtober on October 14, 2022', tools: 'Pencil and Ballpen', image: 'Artworks/Athena.jpg', dateAdded: 'April 25, 2026' }
    ];
    _origSetItem('artworks', JSON.stringify(seedData));
    _origSetItem('artworksVersion', ARTWORK_VERSION);
    artworks = seedData;
  } else {
    ...
    seedData.forEach(seed => {
      ...
      else existing[idx] = { ...existing[idx], ...seed };
    });
...
(function initGooeyNav() {
  ...
  const activateItem = (liEl, index) => {
    ...
    // Burst particles from the li element directly
    filterEl.querySelectorAll('.particle').forEach(p => { try { filterEl.removeChild(p); } catch {} });
    liEl.querySelectorAll('.particle').forEach(p => { try { liEl.removeChild(p); } catch {} });
  ...
  items.forEach((li, index) => {
    li.addEventListener('click', () => activateItem(li, index));
  });

  // Init position on first active item
  const firstActive = items[activeIndex];
  if (firstActive) {
    firstActive.classList.add('active');
    setTimeout(() => {
      updateEffectPosition(firstActive);
      textEl.classList.add('active');
    }, 100);
  }

  // Sync with scroll-based active nav
  const origScrollHandler = window.onscroll;
  window.addEventListener('scroll', () => {
    const current = [...document.querySelectorAll('section[id]')]
      .reverse()
      .find(s => window.scrollY >= s.offsetTop - 100);
    if (!current) return;
    let id = current.getAttribute('id');
    if (id === 'film') id = 'projects';
    const matchIndex = [...items].findIndex(li => li.querySelector('a')?.getAttribute('href') === `#${id}`);
